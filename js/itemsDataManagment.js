import {filterDataByDate, filterDataByCategory, filterDataByType,filterByDate, filterByCategory, filterByType} from "./utils.js";
...
const populateExpenses = (expenses, currentPage) => {
    ...
    const paginatedExpenses = expenses.sort((a, b) => new Date(b.date) - new Date(a.date)).slice(startIndex, endIndex);

    paginatedExpenses.forEach(expense => {
        const expenseElement = createExpenseElement(expense, expenses.indexOf(expense) + 1 + startIndex);
...
function createExpenseElement(expense, id) {
    ...
    <p class="text-gray-500 w-1/12 text-start text-sm flex flex-col"><span class="font-medium">#${id}</span><span class="text-gray-500"> ${expense.date}</span></p>
    ...
    <p class="font-medium w-1/6 text-start text-${typeClass}">${expense.type}</p>
    <p class="font-medium w-1/6 text-end">${expense.type === "Expense" ? "-" : "+"} ${expense.amount}€</p>
...
const populateFilterOptions = () => {
    const categories = expensesData.map(expense => expense.category ).filter((value, index, self) => self.indexOf(value) === index);
    const types = expensesData.map(expense => expense.type).filter((value, index, self) => self.indexOf(value) === index);
